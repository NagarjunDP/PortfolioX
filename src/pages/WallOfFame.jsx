...
const getProjectsCount = (projects) => {
  ...
  if (typeof projects === "string") return projects.split(',').filter(Boolean).length;
  return 0;
};

const getCertificationsCount = (certs) => {
  if (!certs) return 0;
  if (Array.isArray(certs)) return certs.filter(Boolean).length;
  if (typeof certs === "string") return certs.split(',').filter(Boolean).length;
  return 0;
};
...
const computeScore = (data, githubStats) => {
  ...
  const skills = [...(data.skills || []), ...(data.newSkill ? [data.newSkill] : [])];
  score += skills.length * 50;
  score += getProjectsCount(data.projects) * 100;
  score += getCertificationsCount(data.certifications) * 75;
  score += githubStats.repoCount * 20;
  score += githubStats.totalStars * 10;
  score += data.interests?.toLowerCase().includes('ai') ? 50 : 0;
  return score;
          ...
          try {
            if (snapshot.exists()) {
              ...
              for (const [uid, student] of Object.entries(data)) {
                ...
                if (getProjectsCount(student.projects) >= 3) badges.push('Project Pro');
                if (getCertificationsCount(student.certifications) >= 2) badges.push('Certified Expert');
                ...
                leaderboardData.push({
                  ...
                  location: student.location || 'Unknown Location',
                  score,
                  achievements: getCertificationsCount(student.certifications),
                  projects: getProjectsCount(student.projects),
                  hackathons: 0,
                  followers: githubStats.totalStars || 0,
                  verified: true,
                  badges,
